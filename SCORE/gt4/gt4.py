...
class GT4(IconScoreBase):
    ...
    def _transfer(self, _from: Address, _to: Address, _tokenId: int):
        ...
        #self._clear_approval(_tokenId) // retain ownership to contract owner
    ...
    @external
    def mint(self, _to: Address, _tokenId: int, _manufacturer: str, _year: str, _model: str, _country: str, _hp: str, _torque: str, _displacement: str):
        ...
        props = {"manufacturer": _manufacturer, "year": _year, "model": _model, "country": _country, "hp": _hp, "torque": _torque, "displacement": _displacement}
        for prop in props:
            self._theToken[_tokenId][prop] = props[prop]

        self._tokenList.put(_tokenId)  # bug in unit test?
        self._add_tokens_to(_to, _tokenId)
        self._tokenApprovals[_tokenId] =  self.owner # centralized operator to contract owner
    ...
    def get_the_token(self, _tokenId: int) -> dict:
        props = ["manufacturer", "year", "model", "country", "hp", "torque", "displacement"]
        tokenProps = {}
        for prop in props:
